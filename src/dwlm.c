...
typedef struct Client {
	...
	struct wl_list scol; /* link into a scroll column's client list */
	ScrollCol *scol_col; /* NULL when not in a scroll column */
	union {
...
struct Monitor {
	...
	int nmaster;
	ScrollState scroll; /* state for the scroll layout */
	char ltsymbol[16];
	int asleep;
};
...
static struct wl_listener pointerswipebegin_l;
static struct wl_listener pointerswipeupdate_l;
static struct wl_listener pointerswipeend_l;
...
createpointer(struct wlr_pointer *pointer)
{
	...
	if (!swipe_attached) {
		LISTEN(&pointer->events.swipe_begin, &pointerswipebegin_l,
				pointerswipebegin);
		LISTEN(&pointer->events.swipe_update, &pointerswipeupdate_l,
				pointerswipeupdate);
		LISTEN(&pointer->events.swipe_end, &pointerswipeend_l, pointerswipeend);
		swipe_attached = 1;
	}
}
...
mapnotify(struct wl_listener *listener, void *data)
{
	...
	c->border[0] = wlr_scene_rect_create(c->scene, 0, 0,
			c->isurgent ? urgentcolor : bordercolor);
	c->border[0]->node.data = c;
	/* Single rounded "card" behind the surface provides the focus ring */
	wlr_scene_node_lower_to_bottom(&c->border[0]->node);
...
/* DWLM scroll mode: Niri-style horizontally scrolling layout. Kept in its
 * own file for clarity, but compiled into the same translation unit so it
 * can access dwlm.c's internals, in the spirit of upstream dwl. */
#include "scroll.c"
